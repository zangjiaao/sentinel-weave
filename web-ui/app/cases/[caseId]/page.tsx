...
export default async function CaseDetailPage({ params }: { params: { caseId: string } }) {
  ...
  const targetSummaryMap = new Map<string, { key: string; target: string; asset_id: string | null; dst_ip: string | null; alert_count: number; high_signal_count: number }>();
  ...
  return (
    <section>
      ...
      <div className="detail-grid">
        <div className="card">
          ...
          {attackerTargetMap.map((attacker: any) => (
            <div key={attacker.attacker} className="subcard">
              <p className="meta">
                <strong>{attacker.attacker}</strong> · alerts={attacker.alert_count} · high_signal={attacker.high_signal_count}
              </p>
              <p className="meta">
      ...
      <div className="card">
        <h2>攻击告警时间线（最新优先）</h2>
        {attackAlertTimeline.length === 0 ? <p className="meta">暂无攻击告警时间线</p> : null}
        {attackAlertTimeline.length > 0 ? (
          <div className="table-wrap">
            <table className="timeline-table">
              ...
              <tbody>
                {attackAlertTimeline.map((row: any) => (
                  <tr key={row.alert_id}>
