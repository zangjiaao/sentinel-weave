...
- 若当前攻击链尚无案件，先对至少一条代表性告警调用 `alert.detail`，不要只凭 `alert.fetch` 摘要直接建案
...
- 不要为 `assessment.upsert` 使用旧别名：`entity_id`、`case_id`、`case_ids`、`confidence`、`reason`、`first_seen`、`last_seen`
